...
let prevAs = document.getElementsByClassName('prevA')
let prevImgs = document.getElementsByClassName('img-fluid')
let pageTitle = document.getElementById('pageTitle')
let breadCrumb = document.getElementsByClassName('breadcrumb-item active')
let navBtns = document.getElementsByClassName('summerBtn')
...
let projects = {
    summer: [
        ...
        },
        {
            ...
            paras: [
                'I wanted to end this 12 week program with a true programming challenge. Create a program using none of the libraries I learned this summer! I chose to program morse code because I wanted to learn it and after this week\'s challenge, I can now say the following: ',
                '.-- . / .- .-. . / .-.. .. ...- .. -. --. / .. -. / .- / ... .. -- ..- .-.. .- - .. --- -. .-.-.- / .-- .... --- . ...- . .-. / -... .-. . .- -.- ... / - .... . / ... .. -- ..- .-.. .- - .. --- -. / -.-. .- -. / . ... -.-. .- .--. . .-.-.-'
            ]
        }
    ]
}
//This function is for loading previews of the home, contact, and resume page
function loadPrevs() {
    randSummer = projects.summer.slice(1, projects.summer.length)
    shuffle(randSummer)
    j = 0
    if (prevImgs.length == 5) { j++ }
    for (let i = 0; i < prevImgs.length; i++) {
        prevAs[i].setAttribute('href', randSummer[i].site)
        prevImgs[j].src = 'content/' + randSummer[i].img
        prevImgs[j].alt = randSummer[i].title
        j++
    }

}
//This function is called for loading a project page
function loadSumProj(weekNum) {
    thisWeek = weekNum
    randSummer = projects.summer.slice(1, projects.summer.length)
    shuffle(randSummer)
    //Fill Title
    pageTitle.innerHTML = projects.summer[thisWeek].title
    //Fill Breadcrumb
    breadCrumb[0].innerHTML = 'Week ' + projects.summer[thisWeek].weekNum
    //Fill Buttons
    if (thisWeek == projects.summer.length - 1) {
        navBtns[0].setAttribute('href', projects.summer[thisWeek - 1].site)
        navBtns[1].setAttribute('href', 'summer.html')
    }
    else {
        navBtns[0].setAttribute('href', projects.summer[thisWeek - 1].site)
        navBtns[1].setAttribute('href', projects.summer[thisWeek + 1].site)
    }
    //Fill previews
    let j = 0;
    for (let i = 0; i < 4; i++) {
        if (randSummer[j].thisWeek == thisWeek) { j++ }
        prevAs[i].setAttribute('href', randSummer[j].site)
        prevImgs[i].src = 'content/' + randSummer[j].img
        prevImgs[i].alt = randSummer[j].title
        j++
    }
    //Fill Description
    for (let i = 0; i < projects.summer[weekNum].paras.length; i++) {
        let newPara = document.createElement('p')
        let newNode = document.createTextNode(projects.summer[thisWeek].paras[i])
        newPara.appendChild(newNode)
        description.appendChild(newPara)
    }
    //Fill codepen
    codepen.setAttribute('data-slug-hash', projects.summer[thisWeek].codePenTag)
    //Fill github
    github1.setAttribute('aria-label', "Download msautter/" + projects.summer[thisWeek].gitHubRep + " on GitHub")
    github1.href = "https://github.com/msautter/" + projects.summer[thisWeek].gitHubRep + "/archive/master.zip"
    github2.setAttribute('aria-label', "Star msautter/" + projects.summer[thisWeek].gitHubRep + " on GitHub")
    github2.href = "https://github.com/msautter/" + projects.summer[thisWeek].gitHubRep
}
